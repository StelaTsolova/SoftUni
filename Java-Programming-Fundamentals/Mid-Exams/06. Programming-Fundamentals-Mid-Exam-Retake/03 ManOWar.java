import java.util.Arrays;
import java.util.List;
import java.util.Scanner;
import java.util.stream.Collectors;

public class ManOWar_3_6August2019 {
    public static void main(String[] args) {
        Scanner scanner = new Scanner(System.in);
        List<Integer> pirateShip = Arrays.stream(scanner.nextLine().split(">")).map(Integer::parseInt).collect(Collectors.toList());
        List<Integer> warShip = Arrays.stream(scanner.nextLine().split(">")).map(Integer::parseInt).collect(Collectors.toList());
        int maxHealth = Integer.parseInt(scanner.nextLine());
        String[] command = scanner.nextLine().split("\\s+");
        boolean isEnd = false;
        while (!command[0].equals("Retire")) {
            switch (command[0]) {
                case "Fire":
                    int index = Integer.parseInt(command[1]);
                    int damage = Integer.parseInt(command[2]);
                    if (index >= 0 && index < warShip.size()) {
                        warShip.set(index, warShip.get(index) - damage);
                        if (warShip.get(index) <= 0) {
                            isEnd = true;
                            System.out.println("You won! The enemy ship has sunken.");
                        }
                    }
                    break;
                case "Defend":
                    int startIndex = Integer.parseInt(command[1]);
                    int endIndex = Integer.parseInt(command[2]);
                    int damageD = Integer.parseInt(command[3]);
                    if ((startIndex >= 0 && startIndex < pirateShip.size() && (endIndex >= 0 && endIndex < pirateShip.size()))) {
                    for (int i = startIndex; i <= endIndex ; i++) {
                        pirateShip.set(i, pirateShip.get(i) - damageD);
                        if (pirateShip.get(i) <= 0){
                            isEnd = true;
                            System.out.println("You lost! The pirate ship has sunken.");
                            break;
                        }
                    }
                }
                break;
                case "Repair":
                    int indexR = Integer.parseInt(command[1]);
                    int health = Integer.parseInt(command[2]);
                    if (indexR >= 0 && indexR < pirateShip.size()){
                        int section = pirateShip.get(indexR);
                        pirateShip.set(indexR, Math.min(section + health, maxHealth));
                    }
                    break;
                case "Status":
                    double minHealth = maxHealth * 0.2;
                    int sectionsForRepair = (int) pirateShip.stream().filter(e -> e < minHealth).count();
                    System.out.printf("%d sections need repair.%n", sectionsForRepair);
                    break;
            }
            if (isEnd) {
                break;
            }
            command = scanner.nextLine().split("\\s+");
        }
        if (!isEnd) {
            int pirateShipSum = pirateShip.stream().mapToInt(e -> e).sum();
            int warshipSum = warShip.stream().mapToInt(e -> e).sum();
            System.out.printf("Pirate ship status: %d%n", pirateShipSum);
            System.out.printf("Warship status: %d", warshipSum);
        }
    }
}